...
`slotbus-hub` is a standalone binary that bridges HTTP and shared memory. Workers register their routes via HTTP; clients send normal HTTP requests to the hub; the hub dispatches them through SHM and returns the response.

This is useful when you want multiple worker processes behind a single port without each worker running its own HTTP server.
...
```bash
# Start the hub on port 3200
slotbus-hub --port 3200

# With custom request timeout
slotbus-hub --port 3200 --timeout 30
```
...
| `--timeout` | `30` | Request timeout in seconds. If a worker doesn't respond within this window, the hub returns `504 Gateway Timeout`. |

### Worker Registration

Workers register routes on startup via `POST /internal/register`:
...
The hub creates a shared memory region and named events for the worker. The worker then opens the SHM region using the returned `shm_name` and starts its receive loop.

Path parameters use `{param}` syntax. The hub matches incoming requests against registered routes and extracts parameters before dispatching via SHM.
...
The hub provides a unified Server-Sent Events stream at `GET /events`. Workers emit events via `POST /internal/emit`, and the hub multiplexes them into a single stream tagged with the worker's `source` field.
...
When a worker re-registers with the same `name`, the hub automatically removes the old registration and creates a fresh SHM region. This handles worker restarts gracefully without manual cleanup.
